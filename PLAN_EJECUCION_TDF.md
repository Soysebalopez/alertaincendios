...
- [ ] **P1-2 · Web canal bomberos** (solo código) — teaser en landing + página `/cuarteles` (hero + tabla comparativa civilian/fireman + CTA). Agregar a `sitemap.ts`/`robots.ts`. Invocar skill de diseño. **(Al publicar, actualizar el mensaje del bot sin args para linkear a `/cuarteles`.)**
