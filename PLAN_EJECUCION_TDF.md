...
- [x] **P1-4 · Instrumentación `source` (attribution)** — *2026-05-30; columna **aplicada a prod** (`add_subscribers_source`), código en branch.* El bot ahora parsea `/start <payload>` (antes lo descartaba): `cuartel-<slug>`→`source=cuartel:<slug>`, `src-<slug>`→`campaign:<slug>`. Resuelto desde `bot_commands_log` al suscribirse (first-write-wins, sin filas stub con ubicación nula). Columna `source` nullable. **Links de outreach:** `t.me/alertaforestal_bot?start=cuartel-<slug>` y `?start=src-<slug>` (ver `scripts/sql/p1-4-add-source.sql`). ⏭️ La **auto-promoción a fireman desde token** (deep link que promueve sin `/soybombero`) queda en **P4** (necesita tabla cuarteles+tokens).
...
### 🟡 P2 — Robustez / higiene
- [~] **P2-1 · Monitoreo de crons fail-open + dead-man's-switch** — *parcial, en branch `feat/bot-menu-and-deadmanswitch`.* ✅ Dead-man's-switch: `src/lib/healthcheck.ts` pinga `HEALTHCHECK_PING_URL` (env ya cargada por el owner) al final de los 3 returns de éxito de `/api/goes-alerts`; check Healthchecks.io "alertaforestal-goes-alerts" 15m/15m. ⏭️ Falta: enriquecer `clara_cron_health()` con JOIN a `net._http_response` (status HTTP real, no solo "encolado") para el dashboard; y opcional check propio para fires-alerts.
- [x] **FIX bot menu (post-merge)** — *en branch.* El user reportó que el bot "muestra lo mismo". Causa: el menú nativo de Telegram (botón "/") se setea con `setMyCommands` y el código nunca lo llamaba; + `/start` no listaba bomberos. Fix: endpoint `/api/bot/sync-commands` (cron-auth, reejecutable) que registra el menú con soybombero/dejarcuartel + línea de bomberos en `/start`. Tras deploy: disparar con `net.http_get('.../api/bot/sync-commands?secret='||clara_cron_secret())`.
